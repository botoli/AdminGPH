...
export function DashboardWishlistPlan({ freeCash, items, monthName, editable }: DashboardWishlistPlanProps) {
  ...
  return <section className={styles.card} aria-labelledby="dashboard-wishlist-title">
    <div className={styles.header}>
      <div className={styles.heading}>
        <span className={styles.headingIcon}><ShoppingBag aria-hidden="true" /></span>
        <div><h2 id="dashboard-wishlist-title">План хотелок на {monthName}</h2><p>Распределите свободные деньги, не покидая главную.</p></div>
      </div>
      <Link href="/wishlist" className={styles.openLink}>Управление хотелками <ArrowRight /></Link>
    </div>
    ...
    <div className={styles.summary} aria-label="Баланс хотелок">
      ...
      <div className={afterWishlist < 0 ? styles.summaryDanger : styles.summaryPositive}><span>Останется</span><strong>{formatCurrency(afterWishlist)}</strong></div>
    </div>
    <div className={styles.track}><div style={{ width: `${Math.min(100, Math.max(0, selectedTotal / Math.max(freeCash, 1) * 100))}%` }} /></div>

    {!editable ? <p className={styles.readOnly}>Изменения доступны только в текущем месяце.</p> : null}
    {error ? <p className={styles.error} role="alert">{error}</p> : null}

    {active.length === 0 ? <div className={styles.empty}>Активных хотелок пока нет. Добавьте покупку или накопительную цель в разделе «Хотелки».</div> : <div className={styles.columns}>
      <div className={styles.group}>
        <div className={styles.groupTitle}><ShoppingBag /><div><h3>Покупки</h3><p>Выберите то, что хотите зарезервировать на месяц.</p></div></div>
        <div className={styles.list}>
          {purchases.length === 0 ? <p className={styles.groupEmpty}>Обычных покупок пока нет.</p> : purchases.map((item) => {
            ...
            return <article className={`${styles.purchase} ${selected ? styles.selected : ""}`} key={item.id}>
              <button
                type="button"
                className={styles.check}
                aria-label={selected ? `Убрать ${item.title} из плана` : `Выбрать ${item.title} на месяц`}
                aria-pressed={selected}
                disabled={!editable || pending || (!selected && shortage > 0)}
                onClick={() => allocate(item, selected ? 0 : item.amount)}
              >{selected ? <Check /> : null}</button>
              <div className={styles.itemCopy}><strong>{item.title}</strong><span>{selected ? `Выбрано на ${monthName}` : shortage > 0 ? `Не хватает ${formatCurrency(shortage)}` : "Можно выбрать"}</span></div>
              <b>{formatCurrency(item.amount)}</b>
              {editable ? <button type="button" className={styles.textButton} disabled={pending || (!selected && shortage > 0)} onClick={() => allocate(item, selected ? 0 : item.amount)}>{selected ? "Убрать" : "Выбрать"}</button> : null}
            </article>;
          })}
        </div>
      </div>
      ...
      <div className={styles.group}>
        <div className={styles.groupTitle}><PiggyBank /><div><h3>Копилки хотелок</h3><p>Откладывайте частями на крупные цели.</p></div></div>
        <div className={styles.list}>
          {savings.length === 0 ? <p className={styles.groupEmpty}>Накопительных целей пока нет.</p> : savings.map((item) => <SavingsGoal
            key={item.id}
            item={item}
...
function SavingsGoal({ item, allocation, available, editable, pending, onAllocate }: {
  ...
  return <article className={styles.goal}>
    <div className={styles.goalTop}><div><strong>{item.title}</strong><span>Цель {formatCurrency(item.amount)}</span></div><b>{formatCurrency(saved)}</b></div>
    <div className={styles.goalTrack}><div style={{ width: `${Math.min(100, saved / Math.max(item.amount, 1) * 100)}%` }} /></div>
    ...
    {editable && remaining > 0 ? <>
      <div className={styles.goalControls}>
        <label><span>Внести дополнительно</span><div><input type="number" min="1" max={addable} step="1" value={value} onChange={(event) => setValue(event.target.value)} placeholder="0"/><em>₽</em></div></label>
        <button type="button" className={styles.primaryButton} disabled={pending || invalid} onClick={() => { onAllocate(allocation + numericValue); setValue(""); }}>Отложить</button>
      </div>
      <div className={styles.goalActions}>
        <button type="button" disabled={pending || addable <= 0} onClick={() => { setValue(""); onAllocate(allocation + addable); }}>Отложить весь остаток · {formatCurrency(addable)}</button>
        {allocation > 0 ? <button type="button" className={styles.removeButton} disabled={pending} onClick={() => { setValue(""); onAllocate(0); }}>Снять резерв</button> : null}
      </div>
